...
if [ $3 == "quick" ]; then
	...
	INTERDICT=$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/../dicts/web/my-small-dict.txt
elif [ $3 == "slow" ]; then
...
if [[ -f $resultsfolder/../tmp/full-nmap-parsed-tcp.txt ]]; then
	...
	if [[ -f $resultsfolder/../tmp/tcp-tcpwrapped-service.txt ]]; then
		...
		httpx -l $resultsfolder/http-discover/httpx_aux_cleaned.txt -silent -threads 100 -x ALL --retries 5 -status-code | grep -v '.400.' | awk -F' ' '{print $1}' | sort -u > $resultsfolder/full-initial-files.txt
	else
...
do
	...
	cat $resultsfolder/../tmp/ips-with-domains.txt | grep $ip | sed 's/,/\n/g' | sort -ur | ffuf -w $INTERDICT:FUZZ -w -:VHOST -t 20 -noninteractive -p 0.1-0.5 -acc notherexxxxasdf -ac -mc all -u $url/FUZZ -H "Host: VHOST" -se -of all -o $resultsfolder/fuzzing/$(echo $url | sed 's/\/\//-/g'| sed 's/\//-/g' | sed 's/:/-/g') >> $resultsfolder/fuzzing/debug.file 2> $resultsfolder/fuzzing/$(echo $url | sed 's/\//-/g' | sed 's/:/-/g').error
...
cat $resultsfolder/fuzzing/*.json | jq .results[].url | tr -d '"' | sort -u > $resultsfolder/all-urls-fuzzing-results.txt
...
cat $resultsfolder/urls-status-screenshot.txt | aquatone -screenshot-timeout 120000 -threads 50 -http-timeout 120000 --scan-timeout 120000 -out $resultsfolder/screenshots
